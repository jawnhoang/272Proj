<?php include 'header.php'; ?>
<!DOCTYPE html>
<html>

<head>
    <meta charset="UTF-8">
    <link rel="stylesheet" href="src/css/index.css">
    <link rel="stylesheet" href="src/css/style.css">
    <title>JH Kits: Buy and Sell Model Kits!</title>
</head>

<body>
    <div class="cardrow" id="main-page-card">
        <div class="infocard">
            <h1 class='landing-header'>
                Welcome to JH Kits
            </h1>
            <p id='header-p'>Purchase newly imported model kits here!</p>
            <div class='icon-card'>
                <svg class='penguin' height="310" width="300" xmlns="http://www.w3.org/2000/svg">
                    <ellipse id='body' cx="150" cy="150" rx="80" ry="130" fill="black" />
                    <ellipse id='tummy' cx="150" cy="170" rx="58" ry="80" fill="white" />
                    <ellipse id='face' cx="150" cy="70" rx="58" ry="30" fill="white" />
                    <polygon id='beak' points="180,70 120,70 150, 100" fill="yellow" />
                    <ellipse id='Leye' cx="120" cy="73" rx="10" ry="10" fill="black" />
                    <ellipse id='Reye' cx="180" cy="73" rx="10" ry="10" fill="black" />
                    <ellipse id='Lfeet' cx="100" cy="275" rx="40" ry="15" fill="black" stroke="white" />
                    <ellipse id='Rfeet' cx="200" cy="275" rx="40" ry="15" fill="black" stroke="white" />
                    <ellipse id='Larm' cx="68" cy="150" rx="20" ry="60" fill="black" stroke="white" />
                    <ellipse id='Rarm' cx="230" cy="150" rx="20" ry="60" fill="black" stroke="white" />
                </svg>
            </div>
        </div>
    </div>
</body>
<?php include 'footer.php';?>
</html>